...
void Power_State_Machine(Uint16 Receive_Data)
{
	switch(Power_State)
	{
		...
		case CheckSum:
			...
			Power_State = HeadFirst;
			if(Receive_Data == Rdat.checksum)//接收到的数据是正确的
			{
				if(Rdat.header == 0xA5)
				{
...
void ARINC_Decode(Uint32 Dat,Fly_To_Mis_Ctr_Info *Control_Information)
{
	switch(Dat & 0x00ff)
	{
		case F_LAUCH_MISSILE_A429_CONTROL: // 控制字、导弹查询字
...
{
	Uint32  ARINC_Dat;
	ARINC_Dat = (Status_Information->Missile_Normal  	   << ML_NORMAL_BIT)   			|// 导弹是否正常
				(Status_Information->Missile_Capture       << ML_CAPTURE_BIT)  			|// 导弹截获
				(Status_Information->Missile_Type    	   << ML_TYPE_BIT)     			|// 导弹类型
				(Status_Information->Fly_Control_Power     << ML_FLY_CONTROL_POWER_BIT) |// 飞控供电状态
				(Status_Information->RAM_Check    	   	   << ML_RAM_CHECK_BIT)     	|// 外部RAM自检状态
				(Status_Information->Guide_Check     	   << ML_GUIDE_CHECK_BIT) 		|// 导引自检状态
				(Status_Information->IMU_Check    	       << ML_IMU_CHECK_BIT)     	|// IMU自检状态
				(Status_Information->Steering_Check    	   << ML_STEERING_CHECK_BIT)    |// 舵机自检状态
				(Status_Information->Fly_Control_Batter    << ML_FLY_CONTROL_BATTER_BIT)|// 飞控电池状态
				(Status_Information->Steering_Batter   	   << ML_STEERING_BATTER_BIT)   |// 舵机电池状态
				(Status_Information->Engine_Fire_Sigle     << ML_ENGINE_FIRE_SIGLE_BIT) |// 发动机点火信号状态
				MISSILE_LAUCH_A429_STATUS;												 // 标签
